...
public class CamFly : MonoBehaviour
{
    ...
    public float joystickSensitivity = 5.0f; //joystick sensitivity
    ...
    void FixedUpdate()
    {
        //Debug.Log("RightStickX = "+ Input.GetAxis("RightJoyStickX") + " RightStickY = " +Input.GetAxis("RightJoyStickY"));
        // Mouse commands.
        float rotationX = transform.localEulerAngles.y + Input.GetAxis("Mouse X") * mouseSensitivity;
        /*
        if (ignoreMouse)
        {
            rotationX = transform.localEulerAngles.y + Input.GetAxis("RightJoyStickX") * lookSensitivity;
        }
        */
        ...
        if (ignoreMouse)
        {
            rotationY += Input.GetAxis("RightJoyStickY") * lookSensitivity;
        }
        */
    ...
    private Vector3 getDirection()
    {
        ...
        // Strafing enabled only in Fly Mode.
        return p_Velocity;
    }
